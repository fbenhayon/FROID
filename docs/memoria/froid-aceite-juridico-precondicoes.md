...
**Terceira pré-condição, apurada em 11/09/2026 ao substituir o contrato do NR-1: `LEGAL_DOCUMENT_VERSION` é UMA string para o catálogo inteiro.** Mudar o contrato de um produto sobe a versão de todos: os aceites do profissional e da clínica passam a divergir, e `lgpd_consent_version` de todo paciente fica desatualizado — `main.py` responde **403 "Autorização do paciente inativa ou desatualizada"** ao iniciar sessão. Isso só morde com `FROID_LEGAL_ACCEPTANCE_REQUIRED_BR` ligado, que estava desligado em 26/08/2026 — **reconferir antes de cada deploy que mexe em contrato**, porque a conferência vale para a data em que foi feita e não para sempre. Não existe versão por documento: o hash é por documento, a versão não.
