...
export function LiveSession() {
  ...
  const [riskScores, setRiskScores] = useState({ depression: 0, mania: 0, stress: 0, anxiety: 0, dissociation: 0, trauma: 0 });
  ...
  const emotionDescriptions: Record<string, { label: string; canonical_aus: string; description: string }> = {
    happy: { label: 'Felicidade', canonical_aus: 'AU 6 + AU 12', description: 'O "Duchenne Marker" (AU 6) distingue alegria genuina de sorrisos sociais. Sem AU 6, o sorriso e classificado como voluntario.' },
    sadness: { label: 'Tristeza', canonical_aus: 'AU 1 + AU 4 + AU 15', description: 'A elevacao obliqua das sobrancelhas internas (AU 1) + abaixamento dos cantos da boca (AU 15) e quase impossivel de falsificar.' },
    surprise: { label: 'Surpresa', canonical_aus: 'AU 1 + AU 2 + AU 5 + AU 26/27', description: 'Movimento rapido e simetrico. Se durar mais de 1 segundo, e provavel que seja surpresa fingida.' },
    fear: { label: 'Medo', canonical_aus: 'AU 1+2+4+5+7+20+25', description: 'Combina arregalar de olhos (AU 5) com esticamento horizontal dos labios (AU 20), criando tensao multidirecional.' },
    anger: { label: 'Raiva', canonical_aus: 'AU 4 + AU 5 + AU 7 + AU 23/24', description: 'Foco no olhar fixo (AU 5/7) e estreitamento dos labios (AU 23), sinalizando preparacao para agressao ou supressao.' },
    disgust: { label: 'Nojo', canonical_aus: 'AU 9 / AU 10 + AU 17', description: 'O enrugamento do nariz (AU 9) e o sinal mais confiavel. AU 10 e uma versao mais leve ou controlada do nojo.' },
    contempt: { label: 'Desprezo', canonical_aus: 'R12A / R14A (Unilateral)', description: 'Unica emocao universal definida por assimetria. Puxao unilateral do canto da boca e o marcador tecnico do desdém.' },
  };
  ...
  const connectVoiceWebSocket = (sessionId: string) => {
    ...
    ws.onmessage = (event) => {
      ...
      if (data.spectral_bands) setBandData(data.spectral_bands);
      if (data.clinical_scores) {
        setRiskScores({
          depression: Math.round((data.clinical_scores.depression_risk || 0) * 100),
          mania: Math.round((data.clinical_scores.mania_activation || 0) * 100),
          stress: Math.round((data.clinical_scores.stress_cognitive || 0) * 100),
          anxiety: Math.round((data.clinical_scores.anxiety_risk || 0) * 100),
          dissociation: Math.round((data.clinical_scores.dissociation_risk || 0) * 100),
          trauma: Math.round((data.clinical_scores.trauma_risk || 0) * 100),
        });
      }
      if (data.colorimetry_level !== undefined) setColorimetryLevel(data.colorimetry_level);
      if (data.ipm_score !== undefined) {
        const ipmPct = Math.round(data.ipm_score * 100);
        setIpmScore(ipmPct);
        setIpmHistory(prev => [...prev.slice(-19), ipmPct]);
      }
    };
  ...
  const connectFaceWebSocket = (sessionId: string) => {
    const protocol = window.location.protocol === 'https:' ? 'wss:' : 'ws:';
    const ws = new WebSocket(`${protocol}//${window.location.host}/ws/face/${sessionId}`);
  ...
  const startCamera = async () => {
    try {
      ...
      // --- Envio de frames de vídeo para froid-face (3 FPS) ---
      ...
      frameIntervalRef.current = window.setInterval(() => {
        if (!videoRef.current || !faceWsRef.current || faceWsRef.current.readyState !== WebSocket.OPEN) return;
        const ctx = canvas.getContext('2d');
        if (!ctx) return;
        ctx.drawImage(videoRef.current, 0, 0, 640, 480);
        canvas.toBlob((blob) => {
          if (blob && faceWsRef.current?.readyState === WebSocket.OPEN) {
            blob.arrayBuffer().then(buf => faceWsRef.current?.send(buf));
          }
        }, 'image/jpeg', 0.7);
      }, 333);

      // --- Envio de áudio PCM 16-bit para froid-voice ---
    ...
    <div className="min-h-screen bg-gray-900 text-white">
      ...
      <div className="max-w-[1920px] mx-auto p-4 grid grid-cols-12 gap-3">
        ...
        <div className="col-span-5 space-y-3">
          ...
          <div className="bg-gray-800 rounded-lg p-3">
                ...
                <div key={i} className="flex flex-col items-center">
                  <div
                    className={`w-10 h-10 rounded-full transition-all duration-500 ${
                      i === colorimetryLevel ? 'ring-4 ring-white scale-125 shadow-lg' : 'opacity-40 scale-90'
                    }`}
                    style={{
                      backgroundColor: color,
                      boxShadow: i === colorimetryLevel ? `0 0 20px ${color}` : 'none'
                    }}
                  />
                  {i === colorimetryLevel && (
            ...
            <div className="mt-2 text-center">
              ...
              <p className="text-sm font-bold" style={{ color: colorimetryColors[colorimetryLevel] }}>
        ...
        <div className="col-span-4 space-y-3">
          ...
          <div className="bg-gray-800 rounded-lg p-3">
            ...
            <div className="space-y-2">
              {[
                ...
                { key: 'dissociation', label: 'Dissociação' },
                { key: 'trauma', label: 'Trauma' },
              ].map(({ key, label }) => (
                <div key={key}>
                  <div className="flex justify-between text-xs mb-1">
                    <span>{label}</span>
                    <span className="font-semibold">{riskScores[key as keyof typeof riskScores]}%</span>
                  </div>
                  <div className="w-full bg-gray-700 rounded-full h-1.5">
                    <div
                      className={`h-1.5 rounded-full ${getRiskColor(riskScores[key as keyof typeof riskScores])} transition-all duration-500`}
                      style={{ width: `${Math.min(riskScores[key as keyof typeof riskScores], 100)}%` }}
                    />
                  </div>
                </div>
              ))}
          ...
          {/* Emoções FACS */}
          <div className="bg-gray-800 rounded-lg p-3">
            <h3 className="text-sm font-semibold mb-2">😊 Emoções (FACS - EMFACS)</h3>
            <div className="text-center mb-2">
              <p className="text-xs text-gray-400">Atual:</p>
              <div className="flex items-center justify-center gap-2">
                <p className="text-lg font-bold text-green-400">
                  {currentEmotion ? (emotionDescriptions[currentEmotion]?.label || currentEmotion) : 'Aguardando...'}
                </p>
                {currentEmotion && emotionDescriptions[currentEmotion] && (
                  <button
                    onClick={() => setTooltipEmotion(tooltipEmotion === currentEmotion ? null : currentEmotion)}
                    className="w-5 h-5 rounded-full bg-gray-600 hover:bg-blue-600 text-xs font-bold flex items-center justify-center"
                    title="Info cientifica"
                  >?</button>
                )}
              </div>
              {tooltipEmotion && emotionDescriptions[tooltipEmotion] && (
                <div className="mt-2 text-left bg-gray-900 border border-blue-500 rounded p-2 text-xs">
                  <p className="font-bold text-blue-400 mb-1">{emotionDescriptions[tooltipEmotion].label} - {emotionDescriptions[tooltipEmotion].canonical_aus}</p>
                  <p className="text-gray-300">{emotionDescriptions[tooltipEmotion].description}</p>
                </div>
              )}
            </div>
            {/* 7 emoções com ícone de info */}
            <div className="grid grid-cols-2 gap-1 mb-2">
              {Object.entries(emotionDescriptions).map(([key, desc]) => (
                <div key={key} className={`flex items-center justify-between px-1.5 py-0.5 rounded text-xs ${
                  currentEmotion === key ? 'bg-green-900 ring-1 ring-green-500' : 'bg-gray-700'
                }`}>
                  <span className={currentEmotion === key ? 'text-green-400 font-semibold' : 'text-gray-400'}>{desc.label}</span>
                  <button
            ...
            <div className="space-y-1 max-h-20 overflow-y-auto text-xs">
              {emotionHistory.slice(-5).reverse().map((emotion, i) => (
                <div key={i} className="flex justify-between bg-gray-700 rounded px-1 py-0.5">
                  ...
                  <span>{emotionDescriptions[emotion]?.label || emotion}</span>
                </div>
              ))}
            </div>
          </div>
