...
# FICHA DE ACEITE — a identificação que NÃO entra no corpo do documento.
#
# Determinação do dono, 11/09/2026, junto com a troca do TCLE e do contrato do
# NR-1. O texto novo pede, no item 21, que o aceite identifique a pessoa
# atendida, o profissional, sua categoria e registro, a clínica, a data, a
# versão, o hash e o identificador eletrônico do aceite.
#
# Interpolar isso DENTRO do corpo daria um hash diferente para cada pessoa, e o
# `legal_acceptance_events` deixaria de comparar aceites entre si — perderíamos
# justamente a prova que ele existe para dar. Então o corpo fica idêntico para
# todos, e a identificação é produzida aqui, ao lado, a partir do cadastro.
#
# A regra de ouro vale campo a campo: onde o cadastro não tem o dado, a ficha
# DECLARA a ausência. Nada é suposto, e em nenhuma hipótese se apresenta o hash
# do texto de hoje como se fosse o que a pessoa aceitou — esse vem do ledger, ou
# não vem.
FICHA_NAO_REGISTRADO = "não registrado"


def _ficha_valor(valor: object) -> str:
    texto = str(valor or "").strip()
    return texto or FICHA_NAO_REGISTRADO


def _ficha_do_profissional(email: str) -> dict:
    """Nome, categoria, registro e organização de um profissional, para a ficha.

    Categoria e registro vêm de `profile_fields`, que é onde o cadastro os
    grava (`professionalCouncil` e `professionalRegistry`). O gerador de PDF lê
    os mesmos dois campos de uma cópia no `localStorage` do navegador; esta é a
    leitura do servidor, e é a que vale para a prova.
    """
    owner_email = _normalize_email(email)
    perfil = PROFESSIONAL_PROFILES.get(owner_email) if owner_email else None
    perfil = perfil if isinstance(perfil, dict) else {}
    campos = perfil.get("profile_fields")
    campos = campos if isinstance(campos, dict) else {}
    return {
        "profissional_responsavel": _ficha_valor(perfil.get("owner_name") or owner_email),
        "categoria_profissional": _ficha_valor(campos.get("professionalCouncil")),
        "registro_profissional": _ficha_valor(campos.get("professionalRegistry")),
        "clinica_ou_organizacao": _ficha_valor(
            perfil.get("organization_name") or perfil.get("organization_legal_name")
        ),
    }


def _invite_por_id(invite_id: str) -> dict:
    alvo = str(invite_id or "").strip()
    if not alvo:
        return {}
    for invite in SESSION_INVITES.values():
        if isinstance(invite, dict) and str(invite.get("id") or "") == alvo:
            return invite
    return {}


def _aceite_registrado(subject_reference: str, document_key: str) -> dict:
    """O último aceite gravado para um sujeito e um documento.

    Lê do ledger append-only, que é a única fonte do sha256 EFETIVAMENTE aceito.
    Recalcular o hash a partir do catálogo de hoje apresentaria o texto vigente
    como se fosse o texto assinado — e um aceite de versão anterior passaria a
    "provar" um documento que a pessoa nunca viu.
    """
    subject_hash = _legal_hmac(subject_reference)
    if not subject_hash or not TENANT_STORE.enabled:
        return {}
    try:
        ...
        LOGGER.exception("Unable to read legal acceptance ledger for the acceptance sheet")
        return {}
    do_documento = [
        registro
        for registro in registros
        if str(registro.get("document_key") or "") == document_key
    ]
    return do_documento[-1] if do_documento else {}


def _ficha_de_aceite_do_paciente(patient: dict) -> dict:
    """A ficha do item 21 do TCLE, montada do cadastro e do ledger."""
    patient_id = str((patient or {}).get("id") or "")
    entrada = None
    for registro in reversed(CONSENT_LEDGER):
        if str(registro.get("patient_id") or "") == patient_id:
            entrada = registro
            break
    invite = _invite_por_id((entrada or {}).get("invite_id") or "")
    profissional = _ficha_do_profissional(invite.get("professional_email") or "")
    aceite = _aceite_registrado(patient_id, "patient_tcle")
    return {
        "documento": "patient_tcle",
        "pessoa_atendida": _ficha_valor((patient or {}).get("name")),
        **profissional,
        "data_e_hora_do_aceite": _ficha_valor(
            aceite.get("accepted_at") or (entrada or {}).get("accepted_at")
        ),
        "versao_do_termo": _ficha_valor(
            aceite.get("document_version")
            or (entrada or {}).get("version")
            or (patient or {}).get("lgpd_consent_version")
        ),
        # Sem ledger não há hash: o do catálogo é o de HOJE, e apresentá-lo aqui
        # afirmaria que foi ele o aceito.
        "hash_da_versao": _ficha_valor(aceite.get("document_sha256")),
        "identificador_eletronico_do_aceite": _ficha_valor(
            aceite.get("acceptance_id") or (entrada or {}).get("hash")
        ),
        "versao_vigente_do_catalogo": LEGAL_DOCUMENT_VERSION,
    }


def _ficha_de_aceite_da_organizacao(organization_id: str, document_key: str) -> dict:
    """A ficha do fecho do contrato corporativo.

    Local do aceite, representante, cargo e testemunhas NÃO são coletados pelo
    cadastro hoje. Saem declarados como ausentes, e não em branco: campo vazio
    num documento jurídico se lê como "não havia", e o que há é "não perguntamos".
    """
    # A organização de um perfil NÃO é um campo dele: é derivada, e a regra mora
    # em `organization_id_for_profile` (CNPJ para clínica e empresa, e-mail para
    # o autônomo). Minha primeira versão procurava por uma chave
    # `profile["organization_id"]` que não existe — a busca casaria zero perfis
    # e a ficha sairia inteira em "não registrado", sem nada acusando o erro.
    # Recalcular a regra por fora seria duas verdades sobre a mesma coisa.
    alvo = str(organization_id or "")
    perfil = {}
    for candidato in PROFESSIONAL_PROFILES.values():
        if not isinstance(candidato, dict):
            continue
        derivada = tenant_organization_id_for_profile(
            candidato.get("owner_email") or "",
            str(candidato.get("account_type") or "individual"),
            candidato.get("organization_document"),
        )
        if derivada == alvo:
            perfil = candidato
            break
    aceite = _aceite_registrado(alvo, document_key)
    return {
        "documento": document_key,
        "razao_social": _ficha_valor(
            perfil.get("organization_legal_name") or perfil.get("organization_name")
        ),
        "cnpj": _ficha_valor(perfil.get("organization_document")),
        "sede": FICHA_NAO_REGISTRADO,
        "representante": _ficha_valor(perfil.get("owner_name")),
        "cargo": FICHA_NAO_REGISTRADO,
        "local_do_aceite": FICHA_NAO_REGISTRADO,
        "testemunhas": [],
        "data_e_hora_do_aceite": _ficha_valor(aceite.get("accepted_at")),
        "versao_do_contrato": _ficha_valor(aceite.get("document_version")),
        "hash_da_versao": _ficha_valor(aceite.get("document_sha256")),
        "identificador_eletronico_do_aceite": _ficha_valor(aceite.get("acceptance_id")),
        "versao_vigente_do_catalogo": LEGAL_DOCUMENT_VERSION,
    }
...
@app.get("/api/patient-portal/legal-acceptance")
async def patient_portal_legal_acceptance(request: Request):
    """A ficha de aceite do item 21 do TCLE, para a pessoa atendida.

    O corpo do Termo é idêntico para todo mundo — é isso que mantém um hash por
    versão e permite comparar aceites. A identificação individual mora aqui, ao
    lado, montada do cadastro e do ledger append-only.

    Devolve também a íntegra vigente do documento, para que a tela possa
    confrontar o sha256 registrado no aceite com o do texto de hoje. Divergiu, o
    texto vigente não é o texto aceito, e quem lê precisa saber disso em vez de
    receber o documento de hoje sob a data de ontem.
    """
    patient_session = _require_current_patient(request)
    patient_id = str(patient_session.get("id") or "")
    patient = PATIENTS.get(patient_id)
    ...
    ficha = _ficha_de_aceite_do_paciente(patient)
    vigente = catalogo.get("documents", {}).get("patient_tcle", {})
    return {
        "acceptance_sheet": ficha,
        "ledger_configured": _legal_audit_key_configured() and TENANT_STORE.enabled,
        "document": vigente,
        "supplier": catalogo.get("supplier", {}),
        # A comparação sai pronta: a tela não deve refazer a conta, e quem lê o
        # comprovante precisa da resposta, não dos dois números lado a lado.
        "matches_current_version": bool(
            ficha.get("hash_da_versao")
            and ficha.get("hash_da_versao") == vigente.get("sha256")
        ),
    }
...
async def list_organization_legal_acceptances(organization_id: str, request: Request):
    ...
    return {
        ...
        "documents": catalogo.get("documents", {}),
        "supplier": catalogo.get("supplier", {}),
        # A ficha do fecho do contrato corporativo (11/09/2026): a qualificacao
        # das Partes saiu do corpo do documento e passou a ser produzida do
        # cadastro, para que o hash continue o mesmo para todas as contratantes.
        # Campos que o cadastro nao coleta saem DECLARADOS como ausentes.
        "acceptance_sheet": _ficha_de_aceite_da_organizacao(
            context.organization_id, "nr1_company_contract"
        ),
    }
