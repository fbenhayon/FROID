...
def _corpo(chave: str) -> str:
    """O corpo de QUALQUER documento do catalogo.

    A revisao de 13/09/2026 redistribuiu garantias entre os quatro
    documentos do Psique, e varias assercoes deste arquivo passaram a
    precisar olhar para o vizinho.
    """
    return " ".join(s["body"] for s in CATALOGO["documents"][chave]["sections"])
...
class UmContratoParaAsDuasContasClinicas(unittest.TestCase):
    ...
    def test_as_clausulas_de_pessoa_juridica_declaram_o_proprio_escopo(self):
        ...
        """
        self.assertIn(
            "aplicam-se quando a CONTRATANTE for pessoa jurídica", _corpo_do_contrato()
        )
        self.assertIn("quando a CONTRATANTE for o próprio profissional", _corpo_do_contrato())
...
class AAtribuicaoDeFalaEDescritaNaPolitica(unittest.TestCase):
    ...
    # A descricao detalhada saiu do Contrato e foi para a Politica na revisao
    # de 13/09/2026: e materia de tratamento de dados, e a harmonizacao pedida
    # pelo dono manda que cada documento trate da sua. O Contrato e os Termos
    # apontam; a Politica descreve.
    POLITICA = " ".join(
        s["body"] for s in CATALOGO["documents"]["privacy"]["sections"]
    )
    ...
    def test_a_clausula_descreve_o_vetor_e_nao_promete_biometria(self):
        ...
        self.assertIn("não é utilizada para identificação biométrica", nove)

    def test_a_clausula_diz_que_o_vetor_nao_sai_do_navegador(self):
        self.assertIn("armazenamento local no navegador", self.POLITICA)
        self.assertIn("não é transmitida ao FROID", self.POLITICA)
    ...
    def test_a_pessoa_atendida_nao_ganha_assinatura_e_a_atribuicao_e_residual(self):
        ...
        self.assertIn(
            "não constrói representação alguma", self.POLITICA
        )
...
class AClausula13DescreveOQueExisteEOQueNaoExiste(unittest.TestCase):
    ...
    def test_as_tres_ausencias_estao_declaradas(self):
        quatro = _corpo_do_contrato()
        self.assertIn("não contém a transcrição textual integral", quatro)
        self.assertIn("não existe exportação em lote", quatro)
        self.assertIn("não existe exportação geral em formato estruturado", quatro)
...
class AJanelaDeNoventaDiasEstaNoContratoENoCodigo(unittest.TestCase):
    ...
    def test_o_contrato_promete_o_prazo_que_o_codigo_concede(self):
        ...
        self.assertIn(f"janela de {dias} (noventa) dias", _corpo_do_contrato())

    def test_o_contrato_promete_leitura_com_plano_vencido(self):
        self.assertIn("não impedirá imediatamente a leitura dos registros", _corpo_do_contrato())
    ...
    def test_a_janela_nao_promete_aviso_que_ninguem_dispara(self):
        ...
        """
        treze_oito = _corpo_do_contrato()
        self.assertIn("A contagem do prazo é objetiva", treze_oito)
    ...
    def test_a_janela_nao_promete_alterar_nem_apagar(self):
        ...
        self.assertIn("alteração ou exclusão dos registros existentes", _corpo_do_contrato())
...
class AClausula14DizAVerdadeSobreQuemLeOProntuario(unittest.TestCase):
    ...
    def test_a_capacidade_tecnica_nao_e_vendida_como_autorizacao(self):
        self.assertIn("não constitui autorização jurídica ou ética", _corpo_do_contrato())

    def test_a_fronteira_do_empregador_esta_no_contrato_e_no_codigo(self):
        import tenant_access

        self.assertIn("não recebem conteúdo clínico identificado", _corpo_do_contrato())
        ...
        self.assertIn("alcança também a atribuição de pessoas atendidas", _corpo_do_contrato())
...
class AClausula19DeclaraOAcervoEAsQuatroExclusoes(unittest.TestCase):
    ...
    def test_a_fala_desidentificada_do_profissional_integra_o_acervo(self):
        ...
        self.assertIn("fala do PROFISSIONAL após processo de desidentificação", dois)

    def test_as_quatro_exclusoes_continuam_de_pe(self):
        tres = _corpo_do_contrato()
        for excluido in (
    ...
    def test_a_assimetria_e_dita_com_todas_as_letras(self):
        ...
        self.assertIn("não é apresentada como escolha facultativa", quatro)
    ...
    def test_o_descarte_por_ambiguidade_e_registrado_e_nao_silencioso(self):
        ...
        self.assertIn("o descarte é registrado como tal", _corpo_do_contrato())
...
class OsNumerosDoContratoBatemComOsDoProduto(unittest.TestCase):
    ...
    def test_os_tres_segundos_da_janela_acustica(self):
        ...
        self.assertIn("aproximadamente 3 (três) segundos", _corpo_do_contrato())

    def test_o_buffer_nao_e_persistido_e_o_contrato_afirma_isso(self):
        oito = _corpo_do_contrato()
        self.assertIn("não são gravados em disco", oito)
        self.assertIn("não integram backup", oito)
        core = (SERVER_DIR / "froid_core.py").read_text(encoding="utf-8")
    ...
    def test_a_face_manda_numero_e_nao_imagem(self):
        ...
        self.assertIn("dados numéricos derivados necessários à funcionalidade", _corpo_do_contrato())
...
class ATranscricaoNaoCaiParaTextoClaro(unittest.TestCase):
    ...
    def test_a_clausula_promete_falhar_em_vez_de_degradar(self):
        dez = _corpo_do_contrato()
        self.assertIn("não autorizará seu armazenamento em texto claro", dez)
